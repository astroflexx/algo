...
class StringHasher {
  ...
  public:
    StringHasher(const ll p1 = 31, const ll p2 = 37) {
        if (!powers_computed) {
    ...
    pair<ll, ll> hash_string(string &s) {
        ...
        for (ll i = 0; i < n; i++) {
            hash_value1 = (hash_value1 + (s[i] - 'a' + 1) * p1_pow[i]);
            hash_value2 = (hash_value2 + (s[i] - 'a' + 1) * p2_pow[i]);
        }

        return {hash_value1.val(), hash_value2.val()};
    }
};

vector<mint> StringHasher::p1_pow;
vector<mint> StringHasher::p2_pow;
bool StringHasher::powers_computed = false;

// --------------- SET THIS ---------------

ll StringHasher::maxsize = $1;